...
class Progress_Bar extends StatefulWidget {
  @override
  _Progress_BarState createState() => _Progress_BarState();
}

class _Progress_BarState extends State<Progress_Bar> {
  var containerEmptyCArd = <Widget>[];
...
//
  ...
  Widget build(BuildContext context) {
    ...
    Widget emptyText = Padding(
      ...
      child: Text('Вы еще не начали курс.',
    ...
    containerEmptyCArd = <Widget>[emptyCard,emptyText];
    ...
    for (var i = 0; i < availableCorsces.length; i++) {
      var huina = _createCourseCard(
          ...
          height / 30,
          width / 1.3);
      if(AllPgogressMeanings[availableCorsces[i]] > 0 && AllPgogressMeanings[availableCorsces[i]] < 1){
        setState(() {
          containerCard.add(huina);
          empty = false;
          progresCount = containerCard.length;
